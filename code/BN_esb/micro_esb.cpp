...
static void initialize_fifos()
{
    ...
    m_tx_fifo.count       = 0;
    int i;
    for(i = 0; i < UESB_CORE_TX_FIFO_SIZE; i++)
    {
    ...
    m_rx_fifo.count       = 0;
    for(i = 0; i < UESB_CORE_RX_FIFO_SIZE; i++)
    {
...
uint32_t MicroESB::uesb_init(uesb_config_t *parameters)
{
...
uint32_t MicroESB::uesb_disable(void)
{
...
{
    //Serial.println(" write_tx_payload ");
    if(m_uesb_mainstate == UESB_STATE_UNINITIALIZED) 
    {
      return UESB_ERROR_NOT_INITIALIZED;
    }
    
    if(m_tx_fifo.count >= UESB_CORE_TX_FIFO_SIZE) 
    {
      return UESB_ERROR_TX_FIFO_FULL;
    }

    DISABLE_RF_IRQ;
    
    //Serial.println(" write_tx_payload::DISABLE_RF_IRQ ");
    
    if(noack && m_config_local.dynamic_ack_enabled) {
      payload->noack = 1;
    } else {
      payload->noack = 0;
    }
    ...
    //Serial.println(" write_tx_payload::m_tx_fifo.entry_point++; ");
    
    if(m_tx_fifo.entry_point >= UESB_CORE_TX_FIFO_SIZE) {
      m_tx_fifo.entry_point = 0;
    }
    m_tx_fifo.count++;
    ENABLE_RF_IRQ;
...
uint32_t MicroESB::uesb_write_tx_payload(uesb_payload_t *payload)
{
    return write_tx_payload(payload, false);
}

uint32_t MicroESB::uesb_write_tx_payload_noack(uesb_payload_t *payload)
{
    if(m_config_local.dynamic_ack_enabled == 0) 
    {
      return UESB_ERROR_DYN_ACK_NOT_ENABLED;
    }
    return write_tx_payload(payload, true);
}

uint32_t MicroESB::uesb_write_ack_payload(uesb_payload_t *payload)
{
...
uint32_t MicroESB::uesb_read_rx_payload(uesb_payload_t *payload)
{
    if(m_uesb_mainstate == UESB_STATE_UNINITIALIZED) return UESB_ERROR_NOT_INITIALIZED;
...
uint32_t MicroESB::uesb_start_rx(void)
{
    if(m_uesb_mainstate != UESB_STATE_IDLE) return UESB_ERROR_NOT_IDLE;
...
uint32_t MicroESB::uesb_stop_rx(void)
{
...
uint32_t MicroESB::uesb_get_tx_attempts(uint32_t *attempts)
{
    if(m_uesb_mainstate == UESB_STATE_UNINITIALIZED) return UESB_ERROR_NOT_INITIALIZED;
...
uint32_t MicroESB::uesb_flush_tx(void)
{
    if(m_uesb_mainstate != UESB_STATE_IDLE) return UESB_ERROR_NOT_IDLE;
    DISABLE_RF_IRQ;
    m_tx_fifo.count = 0;
...
uint32_t MicroESB::uesb_flush_rx(void)
{
    DISABLE_RF_IRQ;
    m_rx_fifo.count = 0;
    m_rx_fifo.entry_point = 0;
...
uint32_t MicroESB::uesb_get_clear_interrupts(uint32_t *interrupts)
{
    DISABLE_RF_IRQ;
...
uint32_t MicroESB::uesb_set_address(uesb_address_type_t address, const uint8_t *data_ptr)
{
    if(m_uesb_mainstate != UESB_STATE_IDLE) return UESB_ERROR_NOT_IDLE;
    switch(address)
    {
...
uint32_t MicroESB::uesb_set_rf_channel(uint32_t channel)
{
...
uint32_t MicroESB::uesb_set_tx_power(uesb_tx_power_t tx_output_power)
{
    if(m_uesb_mainstate != UESB_STATE_IDLE) return UESB_ERROR_NOT_IDLE;
